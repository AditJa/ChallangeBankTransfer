...
public class AccountsService {
  ...
  @Transactional  
	public void transferBalances(Transfer transfer) throws Exception {
	  ...
	  Account accountFrom = getAccount(transfer.getAccountFromId().toString());
		...
		if(accountFrom.getBalance().compareTo(transfer.getAmount()) < 0) {
			log.info("Account with id:" + accountFrom.getAccountId() + " does not have enough balance to transfer.");
		}
		else {
